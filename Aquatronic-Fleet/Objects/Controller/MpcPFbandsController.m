classdef MpcPFbandsController < matlab.System
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    properties (Nontunable)
        ...
        y_e_bar_ref_max
        y_e_bar_ref_min 
        y_e_bar_max_soft_max 
        y_e_bar_max_soft_min 
        y_e_bar_min_soft_max 
        y_e_bar_min_soft_min 
        ...
        Delta_r_ref_min 
        Delta_r_ref_max 
        Delta_y_e_bar_ref_min 
        Delta_y_e_bar_ref_max 
        Delta_y_e_bar_max_soft_min
        Delta_y_e_bar_max_soft_max
        Delta_y_e_bar_min_soft_min
        Delta_y_e_bar_min_soft_max
    ...
    methods
        function obj = MpcPFbandsController(vessel, params, Ts)
            import casadi.*; 
            ...
            banda_error_y_e_bar = 0.0; % 0.5
            obj.u_ref_max = 1.8;                            % Límite superior para la acción de control u_ref (m/s)
            obj.u_ref_min = 0;                              % Límite inferior para la acción de control u_ref (m/s)
            ...
            obj.y_e_bar_ref_max = banda_error_y_e_bar;      % Límite superior para la acción de control y_e_bar_ref (m)
            obj.y_e_bar_ref_min = -banda_error_y_e_bar;     % Límite inferior para la acción de control y_e_bar_ref (m)
            obj.y_e_bar_max_soft_max = Inf;                 % Límite superior para la acción de control y_e_bar_max_soft (m)
            obj.y_e_bar_max_soft_min = 0;                   % Límite inferior para la acción de control y_e_bar_max_soft (m)
            obj.y_e_bar_min_soft_max = Inf;                 % Límite superior para la acción de control y_e_bar_min_soft (m)
            obj.y_e_bar_min_soft_min = 0;                   % Límite inferior para la acción de control y_e_bar_min_soft (m)
            ...
            obj.Delta_y_e_bar_ref_min = -Inf;               % Límite inferior para los incrementos de acción de control y_e_bar_ref (m)
            obj.Delta_y_e_bar_ref_max = Inf;                % Límite superior para los incrementos de acción de control y_e_bar_ref (m)
            obj.Delta_y_e_bar_max_soft_min = -Inf;          % Límite inferior para los incrementos de acción de control y_e_bar_max_soft (m)
            obj.Delta_y_e_bar_max_soft_max = Inf;           % Límite superior para los incrementos de acción de control y_e_bar_max_soft (m)
            obj.Delta_y_e_bar_min_soft_min = -Inf;          % Límite inferior para los incrementos de acción de control y_e_bar_min_soft (m)
            obj.Delta_y_e_bar_min_soft_max = Inf;           % Límite superior para los incrementos de acción de control y_e_bar_min_soft (m)
            ...
            obj.u_nc(:,4) = zeros(size(obj.u_nc,1),1);
            obj.u_nc(:,5) = zeros(size(obj.u_nc,1),1);
            obj.u_nc(:,6) = zeros(size(obj.u_nc,1),1);
    ...
    methods (Access = private)
        ...
        function [f_model, n_controls, n_states, epsilon] = Dinamica_modelada(obj, vessel)
            ...
            y_e_bar_ref = SX.sym('y_e_bar_ref');            % Referencia virtual en el control de alto nivel para el cross-track error modificado (m)
            y_e_bar_max_soft = SX.sym('y_e_bar_max_soft');  % Variable de restricción blanda para el valor máximo del cross-track error modificado (m)
            y_e_bar_min_soft = SX.sym('y_e_bar_min_soft');  % Variable de restricción blanda para el valor mínimo del cross-track error modificado (m)
            
            controls = [u_ref;u_tar;r_ref;y_e_bar_ref;y_e_bar_max_soft;y_e_bar_min_soft];
        ...
        function [obj_HL] = funcion_objetivo(obj, X_HL, U_HL, P_HL)
            ...
            for k=2:obj.N_p+1
                ...
                if k <= obj.N_c+1 
                    ref_st(2) = U_HL(obj.n_controls,k-1);
                else
                    ref_st(2) = U_HL(obj.n_controls,obj.N_c);
                end
        ...
        function [solver, args] = create_solver(obj, obj_fc, X_HL, U_HL, P_HL)
            ...
            for k = 2:obj.N_p+1  
                ...
                if k<=obj.N_c + 1
                    g_HL = [g_HL ; X_HL(2,k) - U_HL(5,k-1) + U_HL(6,k-1)];   % Variable de estado y_e_bar
                else
                    g_HL = [g_HL ; X_HL(2,k) - U_HL(5,obj.N_c) + U_HL(6,obj.N_c)];   % Variable de estado y_e_bar
                end
            ...
            for k = 2:obj.N_c 
                ...
                g_HL = [g_HL ; U_HL(4,k) - U_HL(4,k-1)];   %#ok<*AGROW> % Incrementos de variables de entrada: Delta_y_e_bar_ref_min <= (y_e_bar_ref(k) - y_e_bar_ref(k-1)) <= Delta_y_e_bar_ref_max
                g_HL = [g_HL ; U_HL(5,k) - U_HL(5,k-1)];   %#ok<*AGROW> % Incrementos de variables de entrada: Delta_y_e_bar_max_soft_min <= (y_e_bar_max_soft(k) - y_e_bar_max_soft(k-1)) <= Delta_y_e_bar_max_soft_max
                g_HL = [g_HL ; U_HL(6,k) - U_HL(6,k-1)];   %#ok<*AGROW> % Incrementos de variables de entrada: Delta_y_e_bar_min_soft_min <= (y_e_bar_min_soft(k) - y_e_bar_min_soft(k-1)) <= Delta_y_e_bar_min_soft_max
            end
            
            args.lbg(obj.n_states*obj.N_p+obj.n_controls-5:obj.n_controls:obj.n_states*obj.N_p+obj.n_controls*obj.N_c-5,1) = obj.Delta_u_ref_min;               % Límite inferior para el incremento de la acción de control u_ref (m/s)
            args.lbg(obj.n_states*obj.N_p+obj.n_controls-4:obj.n_controls:obj.n_states*obj.N_p+obj.n_controls*obj.N_c-4,1) = obj.Delta_u_tar_min;               % Límite inferior para el incremento de la acción de control u_tar (m/s)
            args.lbg(obj.n_states*obj.N_p+obj.n_controls-3:obj.n_controls:obj.n_states*obj.N_p+obj.n_controls*obj.N_c-3,1) = obj.Delta_r_ref_min;               % Límite inferior para el incremento de la acción de control r_ref (rad/s)
            args.lbg(obj.n_states*obj.N_p+obj.n_controls-2:obj.n_controls:obj.n_states*obj.N_p+obj.n_controls*obj.N_c-2,1) = obj.Delta_y_e_bar_ref_min;         % Límite inferior para el incremento de la acción de control y_e_bar_ref (m)
            args.lbg(obj.n_states*obj.N_p+obj.n_controls-1:obj.n_controls:obj.n_states*obj.N_p+obj.n_controls*obj.N_c-1,1) = obj.Delta_y_e_bar_max_soft_min;    % Límite inferior para el incremento de la acción de control y_e_bar_max_soft (m)
            args.lbg(obj.n_states*obj.N_p+obj.n_controls:obj.n_controls:obj.n_states*obj.N_p+obj.n_controls*obj.N_c,1) = obj.Delta_y_e_bar_min_soft_min;        % Límite inferior para el incremento de la acción de control y_e_bar_min_soft (m)
            
            args.ubg(obj.n_states*obj.N_p+obj.n_controls-5:obj.n_controls:obj.n_states*obj.N_p+obj.n_controls*obj.N_c-5,1) = obj.Delta_u_ref_max;               % Límite superior para el incremento de la acción de control u_ref (m/s)
            args.ubg(obj.n_states*obj.N_p+obj.n_controls-4:obj.n_controls:obj.n_states*obj.N_p+obj.n_controls*obj.N_c-4,1) = obj.Delta_u_tar_max;               % Límite superior para el incremento de la acción de control u_tar (m/s)
            args.ubg(obj.n_states*obj.N_p+obj.n_controls-3:obj.n_controls:obj.n_states*obj.N_p+obj.n_controls*obj.N_c-3,1) = obj.Delta_r_ref_max;               % Límite superior para el incremento de la acción de control r_ref (rad/s)
            args.ubg(obj.n_states*obj.N_p+obj.n_controls-2:obj.n_controls:obj.n_states*obj.N_p+obj.n_controls*obj.N_c-2,1) = obj.Delta_y_e_bar_ref_max;         % Límite superior para el incremento de la acción de control y_e_bar_ref (m)
            args.ubg(obj.n_states*obj.N_p+obj.n_controls-1:obj.n_controls:obj.n_states*obj.N_p+obj.n_controls*obj.N_c-1,1) = obj.Delta_y_e_bar_max_soft_max;    % Límite superior para el incremento de la acción de control y_e_bar_max_soft (m)
            args.ubg(obj.n_states*obj.N_p+obj.n_controls:obj.n_controls:obj.n_states*obj.N_p+obj.n_controls*obj.N_c,1) = obj.Delta_y_e_bar_min_soft_max;        % Límite superior para el incremento de la acción de control y_e_bar_min_soft (m)
            
            args.lbx(obj.n_controls-5:obj.n_controls:obj.n_controls*obj.N_c-5,1)  = obj.u_ref_min;               % Límite inferior para la entrada u_ref (m/s)
            args.lbx(obj.n_controls-4:obj.n_controls:obj.n_controls*obj.N_c-4,1)  = obj.u_tar_min;               % Límite inferior para la entrada u_tar (m/s)
            args.lbx(obj.n_controls-3:obj.n_controls:obj.n_controls*obj.N_c-3,1)  = obj.r_ref_min;               % Límite inferior para la entrada r_ref (rad/s)
            args.lbx(obj.n_controls-2:obj.n_controls:obj.n_controls*obj.N_c-2,1)  = obj.y_e_bar_ref_min;         % Límite inferior para la entrada y_e_bar_ref (m)
            args.lbx(obj.n_controls-1:obj.n_controls:obj.n_controls*obj.N_c-1,1)  = obj.y_e_bar_max_soft_min;    % Límite inferior para la entrada y_e_bar_max_soft (m)
            args.lbx(obj.n_controls:obj.n_controls:obj.n_controls*obj.N_c,1)      = obj.y_e_bar_min_soft_min;    % Límite inferior para la entrada y_e_bar_min_soft (m)
            
            args.ubx(obj.n_controls-5:obj.n_controls:obj.n_controls*obj.N_c-5,1)  = obj.u_ref_max;               % Límite superior para la entrada u_ref (m/s)
            args.ubx(obj.n_controls-4:obj.n_controls:obj.n_controls*obj.N_c-4,1)  = obj.u_tar_max;               % Límite superior para la entrada u_tar (m/s)
            args.ubx(obj.n_controls-3:obj.n_controls:obj.n_controls*obj.N_c-3,1)  = obj.r_ref_max;               % Límite superior para la entrada r_ref (rad/s)
            args.ubx(obj.n_controls-2:obj.n_controls:obj.n_controls*obj.N_c-2,1)  = obj.y_e_bar_ref_max;         % Límite superior para la entrada y_e_bar_ref (m)
            args.ubx(obj.n_controls-1:obj.n_controls:obj.n_controls*obj.N_c-1,1)  = obj.y_e_bar_max_soft_max;    % Límite superior para la entrada y_e_bar_max_soft (m)
            args.ubx(obj.n_controls:obj.n_controls:obj.n_controls*obj.N_c,1)      = obj.y_e_bar_min_soft_max;    % Límite superior para la entrada y_e_bar_min_soft (m)
