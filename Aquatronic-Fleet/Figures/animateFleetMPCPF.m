...
function animateFleetMPCPF(hist_X, hist_mlc, tm, m_asv, m_mlc, no_items)
    no_vessels = length(m_asv);
    
    % Vector de tiempo para MLC
    t_mlc = tm(1):m_mlc{1}.T_s:tm(end); % time vector

    p_vec_size = m_mlc{1}.n_states+m_mlc{1}.n_states*m_mlc{1}.N_p+m_mlc{1}.n_controls*m_mlc{1}.N_c+m_mlc{1}.n_controls;
