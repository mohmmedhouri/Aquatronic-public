...
function Figure_mpc_hlc_band(Vessel, hist_X, hist_X_hat, hist_mpc_hlc, tm, t_s, MLC)
...
p_vec_size = MLC.n_states+MLC.n_states*MLC.N_p+MLC.n_controls*MLC.N_c+MLC.n_controls;
p_vec_0 = MLC.n_states+MLC.n_states*MLC.N_p;                                        %Tamaño para sacar las referencias actuales
nm = 1 + (MLC.n_states+MLC.n_states*MLC.N_p+MLC.n_controls*MLC.N_c+MLC.n_controls)+ (MLC.n_controls*MLC.N_c) + 7;             % filas por vehículo en hist_mlc
...
tab5 = uitab(tg, 'Title', 'Y_e_bar');
ax4  = axes('Parent', tab5); hold(ax4,'on'); grid(ax4,'on');
plot(ax4, t_mlc(1:end-1) + Tsam, mpc_hlc(1+p_vec_size+4,1:end-1), 'LineWidth',LineWidth, 'DisplayName','$\bar{y}_e$');
plot(ax4, t_mlc(1:end-1) + Tsam, mpc_hlc(1+p_vec_size+5,1:end-1), 'LineWidth',LineWidth, 'DisplayName','$\bar{y}_{e_{max}}$');
plot(ax4, t_mlc(1:end-1) + Tsam, mpc_hlc(1+p_vec_size+6,1:end-1), 'LineWidth',LineWidth, 'DisplayName','$\bar{y}_{e_{min}}$');
xlabel(ax4, 'Time [s]','Interpreter','latex','FontSize',FontSize);
ylabel(ax4, '$\bar{y}_e$ [m]','Interpreter','latex','FontSize',FontSize);
title(ax4, '\textbf{Limits of error across}','Interpreter','latex','FontSize',FontSize);
legend(ax4, 'Location','best','Interpreter','latex', 'FontSize', FontSize);
...
P_LL = mpc_hlc(2:MLC.n_states+MLC.n_states*MLC.N_p+MLC.n_controls*MLC.N_c+MLC.n_controls+1,indt);
U_opt = reshape((mpc_hlc(MLC.n_states+MLC.n_states*MLC.N_p+MLC.n_controls*MLC.N_c+MLC.n_controls+2:MLC.n_states+MLC.n_states*MLC.N_p+MLC.n_controls*MLC.N_c+MLC.n_controls+MLC.n_controls*MLC.N_c+1,indt))',MLC.n_controls,MLC.N_c)';
...
tab12 = uitab(tg, 'Title', 'Y_e_bar NP');
ax4  = axes('Parent', tab12); hold(ax4,'on'); grid(ax4,'on');
plot(ax4, t_mlc(indt:indt+NP) + Tsam, mpc_hlc(1+p_vec_size+4,indt:indt+NP), 'LineWidth',LineWidth, 'DisplayName','$\bar{y}_e$');
plot(ax4, t_mlc(indt:indt+NP) + Tsam, mpc_hlc(1+p_vec_size+5,indt:indt+NP), 'LineWidth',LineWidth, 'DisplayName','$\bar{y}_{e_{max}}$');
plot(ax4, t_mlc(indt:indt+NP) + Tsam, mpc_hlc(1+p_vec_size+6,indt:indt+NP), 'LineWidth',LineWidth, 'DisplayName','$\bar{y}_{e_{min}}$');
plot(ax4, t_mlc(indt:indt+NP-1) + Tsam, u_opt(:,4), 'LineWidth',LineWidth, 'DisplayName','$\bar{y}_{e_p}$');
plot(ax4, t_mlc(indt:indt+NP-1) + Tsam, u_opt(:,5), 'LineWidth',LineWidth, 'DisplayName','$\bar{y}_{e_{p,max}}$');
plot(ax4, t_mlc(indt:indt+NP-1) + Tsam, u_opt(:,6), 'LineWidth',LineWidth, 'DisplayName','$\bar{y}_{e_{p,min}}$');
xlabel(ax4, 'Time [s]','Interpreter','latex','FontSize',FontSize);
ylabel(ax4, '$\bar{y}_e$ [m]','Interpreter','latex','FontSize',FontSize);
title(ax4, '\textbf{Limits of error across}','Interpreter','latex','FontSize',FontSize);
legend(ax4, 'Location','best','Interpreter','latex', 'FontSize', FontSize);
