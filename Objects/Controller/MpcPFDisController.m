classdef MpcPFDisController < matlab.System
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    methods
        function obj = MpcPFDisController(vessel, params, Ts)
            import casadi.*; 
            ...
            obj.j_min = 0;
            % n_states inicial | n_states*NP referencias | n_controls*NC U_opt | n_controls Controles anteriores | n_disturbances*NP | n_states_ant
            obj.p_vec = zeros(obj.n_states + obj.N_p*obj.n_states + obj.N_c*obj.n_controls + obj.n_controls + obj.N_p*obj.n_disturbances + obj.n_states,1);
            obj.n_ite = 0;
            obj.init = 0;
        end
        ...
        function value = getCurrent_w(obj)
            value = obj.w_ant;
        end
    end
    ...
    methods (Static)
        function xk_hat = get_mpc_hlc(m_hlc)
            ...
            len_pvec = m_hlc{1}.n_states + m_hlc{1}.N_p*m_hlc{1}.n_states + m_hlc{1}.N_c*m_hlc{1}.n_controls + m_hlc{1}.n_controls + m_hlc{1}.N_p*m_hlc{1}.n_disturbances + m_hlc{1}.n_states;
    ...
    methods (Access = private)
        ...
        function [f_model, n_controls, n_states, n_disturbances, epsilon] = Dinamica_modelada(obj, vessel)
            ...
            [spatial_path_CasADI, obj.path] = pathScriptCasADI(w, obj.path_n);
        ...
        function [ff_opt, X_HL, U_HL, Param] = Modelo_predictivo(obj, f_model, Ts)
            ...
            % - Los estados deseados a lo largo del horizonte P_HL(obj.n_states+1:obj.n_states+obj.n_states*obj.N_p)
            % - Los controles deseados a lo largo del horizonte P_HL(obj.n_states+obj.n_states*obj.N_p+1:obj.n_states+obj.n_states*obj.N_p+obj.n_controls*obj.N_c)
            % - Los controles implementados en el instante anterior P_HL(obj.n_states+obj.n_states*obj.N_p+obj.n_controls*obj.N_c+1:obj.n_states+obj.n_states*obj.N_p+obj.n_controls*obj.N_c+obj.n_controls)
            % - Las perturbaciones estimadas a lo largo del horizonte P_HL(obj.n_states+obj.n_states*obj.N_p+obj.n_controls*obj.N_c+1:obj.n_states+obj.n_states*obj.N_p+obj.n_controls*obj.N_c+obj.n_controls+obj.N_p*obj.n_disturbances)
            % - Los estados en el instance anterior P_HL(obj.n_states+obj.n_states*obj.N_p+obj.n_controls*obj.N_c+1:obj.n_states+obj.n_states*obj.N_p+obj.n_controls*obj.N_c+obj.n_controls+obj.N_p*obj.n_disturbances+obj.n_states)

            Param = SX.sym('P_HL',obj.n_states + obj.N_p*obj.n_states + obj.N_c*obj.n_controls + obj.n_controls + obj.N_p*obj.n_disturbances + obj.n_states);
        ...
        function [obj_HL] = funcion_objetivo(obj, X_HL, U_HL, P_HL)
            ...
            for k=2:obj.N_p+1
                ...
                dis = P_HL(idx_dis0 + (k-2)*obj.n_disturbances + 1: idx_dis0 + (k-1)*obj.n_disturbances);       %disturbances
                ...
                cost_st = (zt - ref_st)'*obj.Q*(zt - ref_st);                                             % Coste de etapa de los errores de seguimiento de las referencias de los estados
        ...
        function [solver, args] = create_solver(obj, obj_fc, X_HL, U_HL, P_HL)
            ...
            for k = 2:obj.N_p+1  
                ...
                dis = P_HL(idx_dis0 + (k-2)*obj.n_disturbances + 1: idx_dis0 + (k-1)*obj.n_disturbances); %disturbances
                ROT = [cos(X_HL(3,k)) -sin(X_HL(3,k)) 0;sin(X_HL(3,k)) cos(X_HL(3,k)) 0; 0 0 1];
                nu_c = ROT'*[dis(3)*cos(dis(4)) dis(3)*sin(dis(4)) 0]';

                g_HL = [g_HL ; X_HL(5,k) + nu_c(2)];   % Variable de estado v_bar
            end
    ...
    methods (Access = protected)
        ...
        function refs = stepImpl(obj, u_d, estimation, disturbances_mod)
            import casadi.*; 
            if obj.sincronizador == obj.R_T
                ...
                [xe, ye, v_r_bar] = obj.get_inicial_values(x_hat, y_hat, psi_hat, v_hat, r_hat, V_c_0_mod, beta_c_0_mod);
    
                p_vector = zeros(obj.n_states + obj.N_p*obj.n_states + obj.N_c*obj.n_controls + obj.n_controls + obj.N_p*obj.n_disturbances + obj.n_states,1);
    
                p_vector(1:obj.n_states,1) = [xe ye psi_hat, obj.w_ant, v_r_bar];
                ...
                ff_value_LL = obj.ff_opt(obj.u_nc',p_vector);
                x_opt = full(ff_value_LL)';
                obj.u_nc_l = [obj.u_nc(:,1) x_opt(2:end,3) obj.u_nc(:,3)];
