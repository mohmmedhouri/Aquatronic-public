...
function Figure_mpc_llc(Vessel, hist_X, hist_X_hat, hist_mpc_llc, tm, t_s, LLC)
...
nm = 1 + (LLC.n_states+LLC.n_states*LLC.N_p+LLC.n_controls*LLC.N_c+LLC.n_controls)+ (LLC.n_controls*LLC.N_c) + 3;             % filas por vehículo en hist_mlc
...
plot(ax1, tm, mpc_llc(5,:), 'LineWidth',LineWidth, 'DisplayName','$u_{ref}$');
...
plot(ax2, tm, mpc_llc(7,:), 'LineWidth',LineWidth, 'DisplayName','$r_{ref}$');
...
plot(ax3, tm, mpc_llc(6,:), 'LineWidth',LineWidth, 'DisplayName','$v_{ref}$');
...
P_LL = mpc_llc(2:LLC.n_states+LLC.n_states*LLC.N_p+LLC.n_controls*LLC.N_c+LLC.n_controls+1,indt);
U_opt = reshape((mpc_llc(LLC.n_states+LLC.n_states*LLC.N_p+LLC.n_controls*LLC.N_c+LLC.n_controls+2:LLC.n_states+LLC.n_states*LLC.N_p+LLC.n_controls*LLC.N_c+LLC.n_controls+LLC.n_controls*LLC.N_c+1,indt))',LLC.n_controls,LLC.N_c)';
x_opt = full(LLC.ff_opt(U_opt',P_LL));
u_opt = full(U_opt);

%% 5) Surge ref por ventana de prediccion
tab4 = uitab(tg, 'Title', 'Surge NP');
ax4  = axes('Parent', tab4); hold(ax4,'on'); grid(ax4,'on');
plot(ax4, tm(indt:indt+NP), mpc_llc(5,indt:indt+NP), 'LineWidth',LineWidth, 'DisplayName','$u_{ref}$');
...
plot(ax4, tm(indt:indt+NP), x_opt(1,:), 'LineWidth',LineWidth, 'DisplayName','${u}_{p}$');
...
%% 6) Yaw ref por ventana de prediccion
tab5 = uitab(tg, 'Title', 'Yaw NP');
ax5  = axes('Parent', tab5); hold(ax5,'on'); grid(ax5,'on');
plot(ax5, tm(indt:indt+NP), mpc_llc(7,indt:indt+NP), 'LineWidth',LineWidth, 'DisplayName','$r_{ref}$');
...
plot(ax5, tm(indt:indt+NP), x_opt(3,:), 'LineWidth',LineWidth, 'DisplayName','${r}_p$');
xlim([tm(indt) tm(indt+NP)]);
...
%% 7) Sway por ventana de prediccion
tab6 = uitab(tg, 'Title', 'Sway NP');
ax6  = axes('Parent', tab6); hold(ax6,'on'); grid(ax6,'on');
plot(ax6, tm(indt:indt+NP), mpc_llc(6,indt:indt+NP), 'LineWidth',LineWidth, 'DisplayName','$v_{ref}$');
...
plot(ax6, tm(indt:indt+NP), x_opt(2,:), 'LineWidth',LineWidth, 'DisplayName','${v}_p$');
...
%% 9) Acciones por ventana de prediccion
