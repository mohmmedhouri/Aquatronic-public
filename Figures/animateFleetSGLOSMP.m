...
function animateFleetSGLOSMP(hist_X, hist_mlc, tm, m_asv, m_mlc, no_items, disturbances)
    ...
    p_vec_size = m_mlc{1}.n_states+m_mlc{1}.n_controls+m_mlc{1}.n_disturbances*m_mlc{1}.N_p+3;
    ...
    for t = 1:length(t_mlc)
        for j = 1:no_vessels
            idx = (j-1)*(1+p_vec_size+u_opt_size+4);
            idx2 = (j-1)*2;
